 ...
 */
function wp_auth_jwt_success_response(array $data = [], ?string $message = null, int $status = 200): WP_REST_Response
{
    $response_data = [
        'success' => true,
        'data' => $data
 ...
 */
function wp_auth_jwt_error_response(string $code, string $message, int $status = 400, array $data = []): WP_Error
{
 ...
 */
function wp_auth_jwt_debug_log($message, array $context = []): void
{
    try {
        $settings = [];
