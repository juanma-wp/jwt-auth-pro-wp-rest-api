...
class PluginActivationTest extends WP_UnitTestCase
{
	...
	public function test_deactivation_executes_cleanup_logic(): void
	{
		...
		// Set up test data.
		update_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS, ['test' => 'data']);

		// Verify option exists before deactivation.
		$this->assertNotFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS));

		// Run deactivation - should execute without errors.
		$this->plugin->deactivate();

		// Verify options are deleted.
		$this->assertFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS));
	...
	{
		// Set up options.
		update_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS, [
			'secret_key'           => 'test-secret',
			'access_token_expiry'  => 3600,
			'refresh_token_expiry' => 2592000,
		]);

		update_option(JWT_Auth_Pro_Admin_Settings::OPTION_GENERAL_SETTINGS, [
		...
		$this->assertNotFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS));
		$this->assertNotFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_GENERAL_SETTINGS));
		...
		$this->assertFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS));
		$this->assertFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_GENERAL_SETTINGS));
	...
	public function test_full_activation_deactivation_cycle(): void
	{
		...
		update_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS, ['secret_key' => 'test-cycle']);
		update_option(JWT_Auth_Pro_Admin_Settings::OPTION_GENERAL_SETTINGS, ['debug' => true]);
		...
		$this->assertFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_JWT_SETTINGS), 'JWT settings should be deleted');
		$this->assertFalse(get_option(JWT_Auth_Pro_Admin_Settings::OPTION_GENERAL_SETTINGS), 'General settings should be deleted');
